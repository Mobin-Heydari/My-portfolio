...
export const Vortex = (props: VortexProps) => {
  ...
  const yOff = 0.00125;
  const zOff = 0.0005;
  const backgroundColor = props.backgroundColor || "#222831";
  ...
  const draw = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
    ...
    drawParticles(ctx);
    // (Optional) Additional effects such as glow can be added here.
  ...
  return (
    <div className={cn("relative w-full h-full", props.containerClassName)}>
      {/* Canvas container positioned absolutely */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ref={containerRef}
      ...
      {/* Children container – now absolutely positioned over the canvas */}
      <div
        className={cn(
